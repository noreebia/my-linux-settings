...
Assess feedback on a document and produce a considered response — what's valid, what isn't, and what to do next.
...
Inline (paste the feedback as the argument):

    /assess-feedback The plan looks solid overall but there are gaps in the error handling
    strategy. Specifically: 1) The retry logic doesn't account for...

    /assess-feedback ## Issues\n- Auth flow assumes single-tenant but we're multi-tenant\n- Missing rate limiting on the upload endpoint...

From a file:

    /assess-feedback --file-path=agents/claude/reviews/auth-plan-review.md
    /assess-feedback --file-path=agents/claude/reviews/auth-plan-review.md --file
...
If `--file-path` was given, read the feedback document. Otherwise, use the feedback already present in the conversation context — the user may have pasted it inline. Also read the original document being discussed if available — you need both sides to evaluate fairly.
...
- **Inline** (default): Output directly in the conversation. Omit the metadata header (`*Assessed: ... | Author: ...*`).
- **File** (if `--file` was given): Include the metadata header: `*Assessed: $CURRENT_TIME("YYYY-MM-DD HH:MM") | Author: $AGENT_NAME | Feedback: <path to feedback doc> | Original: <path to original doc if known>*`. Write to `$AGENT_LOCAL_DIR/reviews/<feedback-file-basename>-response.md`. Tell the user where it was saved.
