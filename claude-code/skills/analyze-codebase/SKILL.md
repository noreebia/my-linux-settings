...
Perform a thorough analysis of the current codebase and produce developer-friendly documentation in a `$DOCS_BASE/system-analysis` folder at the repository root. The depth and breadth of the output must match the actual complexity of the project — no more, no less.
...
For each section you decided to include, create or update markdown files inside `$DOCS_BASE/system-analysis` at the repo root.
